"""
Docker GAM - Execute commands in a Docker container
"""
import os
import re
import tarfile
...
class DockerWorkspace(BaseWorkspace):
    """
    Docker gam for executing commands in a Docker container.
    """
    ...
    def __init__(
        ...
        logger=None,
        name: str = "docker_gam",
        description: str = "Docker agent file system for long context task",
        **docker_kwargs,
    ):
        ...
        self.logger.info(f"Docker agent file system initialized")
    ...
    def start_container(self, docker_image: str, command: str, container_name: str, **docker_kwargs):
        ...
        env_vars = {
            ...
            **docker_kwargs.get("agent file system", {})
        }
        ...
        self.container = self.client.containers.run(
            ...
            **{k: v for k, v in docker_kwargs.items() if k != "agent file system"},
        )
        self.logger.info(f"Started container: {container_name}")

    def setup_workspace(self, name: str, description: str):
        """Setup the container agent workspace."""
        # Ensure working directory exists
        self.run(f"mkdir -p {self.root_path}")
        ...
        self.run(f"cat > .gam_meta.json << 'EOF'\n{meta_json}\nEOF")
    ...
    def run(
        ...
        """
        exec_workdir = self.root_path if workdir is None else workdir
